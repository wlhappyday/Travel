...
class User
{
    ...
    public function update(){
        ...
        if(Juser::where(['phone'=>$data['phone'],'status'=>'0'])->value('id')){
        ...
        Db::startTrans();
        try {
            Juser::where(['id'=>$uid,'status'=>'0'])->update($data);
            addJuserLog(getDecodeToken(),'修改用户昵称：'.$data['nickname'].'，用户手机号：'.$data['phone']);
            Db::commit();
    ...
    public function updPasswd(){
        ...
        $userDate = Juser::where(['id'=>$uid,'status'=>'0'])->find();
        ...
        try {
            ...
            Juser::where(['id'=>$uid,'status'=>'0'])->save($data);
            addJuserLog(getDecodeToken(),'用户 '.$userDate['phone'].' 修改密码：');
            Db::commit();
            return returnData(['msg'=>'操作成功','code'=>'200']);
        }catch (\Exception $e){
            Db::rollback();
