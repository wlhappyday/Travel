...
class Order
{
    public function list(){
        ...
        if ($start_time){
            $order_result->whereTime('a.add_time', '>=', strtotime($start_time));
        }
        $end_time = input('post.end_time/s','','strip_tags');
        if ($end_time){
            $order_result->whereTime('a.add_time', '<=', strtotime($end_time));
        }
    ...
    public function listDetail(){
        ...
        $order_result = new Orders();
        if(!$order_result->where(['store_id'=>$store_id,'store_type'=>'1','order_id'=>$order_id])->value('order_id')){
