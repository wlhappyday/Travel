...
class TestUser
{
    ...
    public function add(){
        ...
        $rule = [
            'user_name' => 'require|unique:JTestUser',
            'passwd' => 'require',
            'phone' => 'require|unique:JTestUser|number|max:11|min:11',
//            'phone' => 'require|unique:JTestUser|number|max:11|min:11|mobile',
            'status' => 'require|in:1,9',
        ];
        $msg = [
        ...
        Db::startTrans();
        try {
            JtestUser::insert($data);
            addJuserLog(getDecodeToken(),'添加验票人：'.$data['user_name'].' 手机号: '.$data['phone']);
            Db::commit();
...
//            'phone' => 'require|unique:JTestUser|number|max:11|min:11|mobile',
        ...
        $phone = JtestUser::where(['id'=>$data['id']])->value('phone');
        if($data['phone'] == $phone){
            unset($data['phone']);
            unset($rule['phone']);
        }
        ...
        try {
            $uid = $data['id'];
            unset($data['id']);
            JtestUser::where(['id'=>$uid])->update($data);
            addJuserLog(getDecodeToken(),'修改验票用户信息：'.$phone);
            Db::commit();
            return returnData(['msg'=>'操作成功','code'=>'200']);
        }catch (\Exception $e){
            Db::rollback();
    ...
    public function list(){
        ...
        $where['uid'] = getDecodeToken()['id'];

        $user_result = new JtestUser();
        $data = $user_result->where($where)
            ->order('id desc')
            ->field('id,user_name,phone,status,create_time')
            ->paginate($num);
    ...
    public function updStatus(){
        ...
        Db::startTrans();
        try {
            JtestUser::where(['id'=>$uid])->save($data);
            addJuserLog(getDecodeToken(),$info);
            Db::commit();
            return returnData(['msg'=>'操作成功','code'=>'200']);
        }catch (\Exception $e){
            Db::rollback();
    ...
    public function isDelete(){
        ...
        try {
            JtestUser::where(['id'=>$uid])->save($data);
            addJuserLog(getDecodeToken(),'删除验票方 '.$uid);
            Db::commit();
            return returnData(['msg'=>'操作成功','code'=>'200']);
        }catch (\Exception $e){
            Db::rollback();
