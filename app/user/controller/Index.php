...
class Index
{
    public function index(){
        ...
        $data['user'] = Puser::where(['a.id'=>$id])->alias('a')
            ->join('file b','b.id=a.avatar','LEFT')
            ->field('a.user_name,a.phone,b.file_path')
            ->find();

        $data['user']['sum'] = Puseruser::where(['puser_id'=>$id])->count();
        $data['user']['sum_distcenter'] = Puseruser::where(['puser_id'=>$id,'is_distcenter'=>'1'])->count();
        ...
        $data['week']['pay_sum'] = Cache::store('redis')->get('platform_pay_sum_'.$id);
        $data['week']['pay_amount'] = Cache::store('redis')->get('platform_pay_amount_'.$id);
        $data['week']['refund_amount'] = Cache::store('redis')->get('platform_refund_sum_'.$id);
        $data['week']['refund_sum'] = Cache::store('redis')->get('platform_refund_amount_'.$id);
    ...
    public function  today(){
        ...
        $today_pay_amount = Order::where($where)->whereIn('order_status',[3,6])->where('add_time','>',$begintime)->sum('order_amount');
        $data['today']['pay_sum'] = Order::where($where)->whereIn('order_status',[3,6])->where('add_time','>',$begintime)->count();
        $data['today']['refund_amount'] = Order::where($where)->whereIn('order_status',[4,5])->where('add_time','>',$begintime)->sum('refund_price');;
