...
use app\common\model\Juser as J_user;
...
class Juser
{
    ...
    public function add(){
        ...
        $rule = [
            'user_name' => 'require|unique:j_user',
            'passwd' => 'require',
            'phone' => 'require|unique:j_user|number|max:11|min:11',
//            'phone' => 'require|unique:j_user|number|max:11|min:11|mobile',
        ...
        try {
            J_user::insert($data);
            addAdminLog(getDecodeToken(),'添加景区：'.$data['user_name'].' 手机号: '.$data['phone']);
            Db::commit();
...
//            'phone' => 'require|unique:j_user|number|max:11|min:11|mobile',
        ...
        $phone = J_user::where(['id'=>$data['id']])->value('phone');
        if($data['phone'] == $phone){
            unset($data['phone']);
            unset($rule['phone']);
        }
        ...
        try {
            $uid = $data['id'];
            unset($data['id']);
            J_user::where(['id'=>$uid])->update($data);
            addAdminLog(getDecodeToken(),'修改景区用户信息：'.$phone);
            Db::commit();
            return returnData(['msg'=>'操作成功','code'=>'200']);
        }catch (\Exception $e){
            Db::rollback();
    ...
    public function isDelete(){
        ...
        try {
            J_user::where(['id'=>$uid])->save($data);
            addAdminLog(getDecodeToken(),'删除景区用户 '.$uid);
            Db::commit();
            return returnData(['msg'=>'操作成功','code'=>'200']);
        }catch (\Exception $e){
            Db::rollback();
