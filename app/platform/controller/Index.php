...
class Index
{
    public function index(){
        ...
        $data['user'] = Padmin::where(['a.id'=>$id])->alias('a')
            ->join('file b','b.id=a.avatar','LEFT')
            ->field('a.user_name,a.phone,b.file_path,a.amount,a.money')
            ->find();
    ...
    public function  today(){
        ...
        $today_pay_amount = Order::where($where)->whereIn('order_status',[3,6])->where('add_time','>',$begintime)->sum('p_price*goods_num');
        $data['today']['pay_sum'] = Order::where($where)->whereIn('order_status',[3,6])->where('add_time','>',$begintime)->count();
        $data['today']['refund_amount'] = Order::where($where)->whereIn('order_status',[4,5])->where('add_time','>',$begintime)->sum('p_price*refund_num');;
