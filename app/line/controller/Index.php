...
class Index
{
    public function index(){
        ...
        $data['user'] = Xuser::where(['a.id'=>$id])->alias('a')
        ...
        $data['week']['pay_sum'] = Cache::store('redis')->get('line_pay_sum_'.$id);
        $data['week']['pay_amount'] = Cache::store('redis')->get('line_pay_amount_'.$id);
        $data['week']['refund_amount'] = Cache::store('redis')->get('line_refund_sum_'.$id);
        $data['week']['refund_sum'] = Cache::store('redis')->get('line_refund_amount_'.$id);
